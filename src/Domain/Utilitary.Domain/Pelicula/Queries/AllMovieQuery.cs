...
namespace Ibero.Services.Utilitary.Domain.Libros.Queries
{
    ...
    public class AllMovieQuery : IRequest<object>
    {
        public string Lenguaje { get; set; }
        public int id { get; set; }
        public class Handler : IRequestHandler<AllMovieQuery, object>
        {
            ...
            public async Task<object> Handle(AllMovieQuery request, CancellationToken cancellationToken)
            {
                var response = new object();
                try
                {
                    var JsonLibrosListOnly = await externalService.GETExternalServiceToken(options.Urlthemoviedb+  + request.id +  "/lists?" + options.ClaveAPIthemoviedb + "&language=en-"+ request.Lenguaje + "&page=1", "Bearer" + " " + options.Tokenthemoviedb);
