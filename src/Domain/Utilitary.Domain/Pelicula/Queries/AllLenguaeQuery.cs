...
namespace Ibero.Services.Utilitary.Domain.Pelicula.Queries
{
    ...
    public class AllLenguaeQuery: IRequest<object>
    {
        public string Lenguaje { get; set; }

        public class Handler : IRequestHandler<AllLenguaeQuery, object>
        {
            ...
            public async Task<object> Handle(AllLenguaeQuery request, CancellationToken cancellationToken)
            { 
                var response = new object();            
                try
                {
                    var JsonLibrosListOnly = await externalService.GETExternalServiceToken(options.Urlengiaje  , "Bearer" + " " + options.Tokenthemoviedb);
                    response = JsonConvert.DeserializeObject(JsonLibrosListOnly.ToString());
                }
                catch (Exception ex)
                {
                    throw new DeleteFailureException(nameof(AllLenguaeQuery), ex.Message, ex.Message);
                }
                return response;
            }
        }
